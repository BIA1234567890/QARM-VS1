...
def main():
    ...
    page = st.sidebar.radio(
        "Go to",
        ("About us", "Our investment approach", "Phi assistant", "Portfolio optimization", "Become a Client", "Our Team"),
    )
...
def page_portfolio_optimization(data):
    ...
    # ============================================================
    st.markdown("### Step 5 – Backtest and Today's Portfolio")
    ...
    with col_btn2:
        run_today_clicked = st.button(
            "Compute today's optimal portfolio",
            type="primary",
            disabled=bool(constraint_errors),
            use_container_width=True,
        )
    ...
    if run_today_clicked:
        ...
        st.success("Today's optimal portfolio computed.")
    ...
    if "backtest_results" in st.session_state:
        ...
        with tab_today:
            ...
            if today_res is None:
                st.info(
                    "No current portfolio yet. Click **'Compute today's optimal portfolio'** "
    ...
    st.markdown(
        ...
        <div class="hero-ai">
            ...
            <div class="hero-ai-text">
                Your personal quantitative guide — helping you understand the platform, financial concepts,
                and market context with clarity and professionalism.
            </div>
        </div>
        """,
        unsafe_allow_html=True
        ...
        st.session_state.ai_messages = [
            {
                ...
                "content": (
                    ...
                    "- The app also shows Current Optimal Allocation with:\n"
                    "  - Top holdings\n"
...
def img_to_base64(path):
    img = Image.open(path)
    buf = io.BytesIO()
    img.save(buf, format="jpg")   # IMPORTANT: JPEG here
...
def page_our_team():
    ...
    # TEAM HTML  (FIXED — NO INDENT BEFORE <div>)
    html = f"""
<div class="team-grid">
